...
# 2018-04-17 Jorgen van der Meulen - Added performance data and optimized XI's Gauge Dashlet
...
#Jorgen van der Meulen (Conclusion Xforce)
...
if ( $aStatus{'wsrep_cluster_status'} eq "Primary" ) {
	...
	if ( $aStatus{'wsrep_cluster_size'} == $pNodes ) {
		print "OK wsrep_cluster_size: " . $aStatus{'wsrep_cluster_size'} . ", wsrep_cluster_status: " . $aStatus{'wsrep_cluster_status'} . "|cluster_size=" .  $aStatus{'wsrep_cluster_size'} . "nodes;" . $pNodes . ":;" . $pNodes . ":;0;" . $lGaugeMax . "\n";
		$dbh->disconnect;
		exit($lError{'OK'});
	}
	else {
		print "Caution: wsrep_cluster_size: " . $aStatus{'wsrep_cluster_size'} . ", wsrep_cluster_status: " . $aStatus{'wsrep_cluster_status'} . "|cluster_size=" .  $aStatus{'wsrep_cluster_size'} . "nodes;" . $pNodes . ":;" . $pNodes . ":;0;" . $lGaugeMax . "\n";
		$dbh->disconnect;
		exit($lError{'Warning'});
	}
}
...
print "Galera Cluster for MySQL/MariaDB seems OK...";
exit($lError{'OK'});
